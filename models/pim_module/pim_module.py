...
class MetaLayer(nn.Module):
    def __init__(self, 
        ...
        self.meta_head_1 = nn.Sequential(
                                        nn.Linear(meta_dim, self.proj_size),
                                        nn.ReLU(inplace=True),
                                        nn.LayerNorm(self.proj_size),
                                        ResNormLayer(self.proj_size),
                                        )
    def forward(self, x):
        x = x.flatten(1)
        return self.meta_head_1(x)
...
class ConvCombiner(nn.Module):
    ...
    def __init__(self, 
        ...
        self.conv1d_1 = nn.Conv1d(self.proj_size, self.proj_size, 3)
        self.mp_1 = nn.MaxPool1d(kernel_size= 3, stride = 2)
...
class GCNCombiner_2(nn.Module):
    ...
    def __init__(self, 
        ...
        super(GCNCombiner_2, self).__init__()
        ...
        # # 2
        self.pool2 = nn.Linear(num_joints, num_joints)

        A = torch.eye(num_joints)/100 + 1/100
        self.adj2 = nn.Parameter(copy.deepcopy(A))
        self.conv2 = nn.Conv1d(self.proj_size, self.proj_size, 1)
        self.batch_norm2 = nn.BatchNorm1d(self.proj_size)
        
        self.conv_q2 = nn.Conv1d(self.proj_size, self.proj_size//4, 1)
    ...
    def forward(self, x):
        ...
        hs = self.pool2(hs)
        q2 = self.conv_q2(hs).mean(1)
        k2 = self.conv_k2(hs).mean(1)
        A2 = self.tanh(q2.unsqueeze(-1) - k2.unsqueeze(1))
        A2 = self.adj2 + A2 * self.alpha2
        hs = self.conv2(hs)
        hs = torch.matmul(hs, A2)
        hs = self.batch_norm2(hs)
...
class LSTMCombiner(nn.Module):
    ...
    def __init__(self, 
        ...
        self.lstm = nn.LSTM(self.proj_size, self.proj_size, num_layers = 1, batch_first=True)
        self.batch_norm1 = nn.BatchNorm1d(self.proj_size)
...
class LSTMCombiner_2(nn.Module):
    ...
    def __init__(self, 
                 ...
                 proj_size: Union[int, None] = None,
                 fpn_size: Union[int, None] = None):
        super(LSTMCombiner_2, self).__init__()
        ...
        self.lstm = nn.LSTM(self.proj_size, self.proj_size, num_layers = 2, batch_first=True)
        self.batch_norm1 = nn.BatchNorm1d(self.proj_size)
...
class MLPCombiner(nn.Module):
    ...
    def __init__(self, 
        ...
        self.mlp1 = nn.Linear(total_num_selects,self.proj_size)
        self.relu=nn.ReLU()
...
class MLPCombiner_2(nn.Module):
    ...
    def __init__(self, 
                 ...
                 proj_size: Union[int, None] = None,
                 fpn_size: Union[int, None] = None):
        super(MLPCombiner_2, self).__init__()
        ...
        ##2
        self.mlp2 = nn.Linear(self.proj_size,self.proj_size)
        self.relu2 = nn.ReLU()
...
class BaseCombiner(nn.Module):
    ...
    def __init__(self, 
        ...
        super(BaseCombiner, self).__init__()
        ...
        self.classifier = nn.Linear(total_num_selects, num_classes)
...
class PluginMoodel(nn.Module):
    ...
    def __init__(self, 
        ...
        * num_selects:
        ...
        self.metainfo_dims = metainfo_dims
        ...
        if self.use_combiner:
            ...
            if self.use_fpn:
                ...
                for key, value in outs.items():
                    a, b, c = value.shape
                    metalayer_input[key] = torch.randn(a, len(metainfo_dims), c)
            else:
                for key, value in outs.items():
                    a, b, c = value.shape
                    outs[key] = torch.randn(a, b + metainfo_dims.shape[0], c)
            ...
            if metainfo_dims is not None :
                total_num_selects += 4*len(metainfo_dims)
            
            self.metalayer = MetaLayer(metainfo_dims[0], metalayer_input, comb_proj_size, self.fpn_size)

            assert comb_type in ["MLP_1", "MLP_2", "GCN_1", "GCN_2", "LSTM_1", "LSTM_2", "BASE"], \
            "combiner type {} were not support yet, please choose type 'MLP_1' or 'MLP_2' or 'GCN_1' or 'GCN_2' or 'LSTM_1' or 'LSTM_2' or 'BASE'".format(comb_type)
            if comb_type == "GCN_1":
                self.combiner = GCNCombiner(total_num_selects, num_classes, gcn_inputs, gcn_proj_size, self.fpn_size)
            elif comb_type == "GCN_2":
                self.combiner = GCNCombiner_2(total_num_selects, num_classes, gcn_inputs, gcn_proj_size, self.fpn_size)
            elif comb_type == "MLP_1":
                self.combiner = MLPCombiner(total_num_selects, num_classes, gcn_inputs, gcn_proj_size, self.fpn_size)
            elif comb_type == "MLP_2":
                self.combiner = MLPCombiner_2(total_num_selects, num_classes, gcn_inputs, gcn_proj_size, self.fpn_size)
            elif comb_type == "LSTM_1":
                self.combiner = LSTMCombiner(total_num_selects, num_classes, gcn_inputs, gcn_proj_size, self.fpn_size)
            elif comb_type == "LSTM_2":
                self.combiner = LSTMCombiner_2(total_num_selects, num_classes, gcn_inputs, gcn_proj_size, self.fpn_size)
            elif comb_type == "BASE":
                self.combiner = BaseCombiner(total_num_selects, num_classes, gcn_inputs, gcn_proj_size, self.fpn_size)
    ...
    def forward(self, x: torch.Tensor):
        ...
        logits = {}
        # B C H W
        without_meta, metainfo = torch.split(x, split_size_or_sections=[self.img_size, 1], dim = 3)
        without_meta = self.forward_backbone(without_meta)

        if self.use_fpn:
            without_meta = self.fpn(without_meta)
            self.fpn_predict(without_meta, logits)


        if self.use_selection:
            selects = self.selector(without_meta, logits)
        ...
        if self.use_combiner:
            for key, value in selects.items():
                ...
                # print(value.shape)
                cnt = 0
                if self.metainfo_dims is not None:
                    for dim in self.metainfo_dims:
                        expanded_meta = F.interpolate(metainfo[:, :, :, :1], size=(1,w),mode='bilinear').reshape(b, -1, w)[:,0:1,:]
                        # expanded_meta = self.metalayer(metainfo[:, 0:1, cnt:cnt+dim, 0:1]).unsqueeze(1)

                        value = torch.cat([value, expanded_meta], dim = 1)
                        cnt += dim
                    selects[key] = value
